...
const SAVING_DIR = FORMAL ? SAVING_DIR_HOME+'/formal_v5_shortv3' : SAVING_DIR_HOME+'/testing_v5_shortv3';
...
const STIM_SOURCE = 'stim/27movies_rev/';
const EXPT_N = 65;

const INSTR_PRAC_LIST = ['5994_push', '5801_hug', '6005_throw'];

// const exptVerToManuallyAssign = [
//     10,
//     27
// ]

// const subjNumBefore = 83;
...
function import_json(num){
    ...
    subj.exptVer = exptVer;
    test.exptVer = exptVer;
...
function PREPARE_TRIAL() {
    ...
    //test.trialN = Object.keys(test.trialInput).length; //YY: uncomment to run on local
}
...
function SHOW_EXAMPLE_ANIMATION() {
    ...
    $('#instrVid')[0].play();
    buffer_video($('#bufferVid1')[0], "stim/" + INSTR_PRAC_LIST[0] + STIM_TYPE); // load practice trial's videos
...
function SHOW_PRACTICE() {
    ...
    // before clicking, show the last frame
    $('#vid1')[0].onloadedmetadata = (event) => {
        $('#vid1')[0].currentTime = $('#vid1')[0].duration;
    };
    $('#vid2')[0].onloadedmetadata = (event) => {
        $('#vid2')[0].currentTime = $('#vid2')[0].duration;
    };
    $('#vid3')[0].onloadedmetadata = (event) => {
        $('#vid3')[0].currentTime = $('#vid3')[0].duration;
    };
...
function instr_practice_play(ele) {
    ...
    target.one('ended', instr_practice_check_play_count);
    target[0].play();
    instr.vidPlayCounts[$(ele.target).attr('id')] = instr.vidPlayCounts[$(ele.target).attr('id')] + 1;
}
...
function SHOW_CONSENT() {
    ...
    $('#vid1')[0].currentTime = $('#vid1')[0].duration; // before clicking, show the last frame
    $('#vid2')[0].currentTime = $('#vid2')[0].duration;
    $('#vid3')[0].currentTime = $('#vid3')[0].duration;
}
...
function PLAY(ele) {
    ...
    let target = $(ele.target).closest('.vid');
    target[0].currentTime = 0;
    target.one('ended', CHECK_PLAY_COUNT);
    target[0].play();
