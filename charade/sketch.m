...
% survey using echostring
response = {};

questions = {
    'Gender (f/m):'
    'Age: '
    'Did you find the instructions clear and easy to follow? (y/n)'
    'Were the stimuli presented appropriately? (y/n)'
};

% Present survey questions
for q = 1:length(questions)
    % propmt size
    subjIDPromptDimRect = Screen('TextBounds', w, questions{q}) ;                % rect for ID prompt
    ...
    responseRecord = GetEchoString(w, questions{q}, xSubjIDPrompt, ySubjIDPrompt, [1 1 1], [0 0 0]);
    response = [response, responseRecord];
